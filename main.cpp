...
int main() {
	...
	const Board startHashValue = Board();
	{
		...
		m[startHashValue] = start;
		updateQueue.push(startHashValue);
	}

	////////////ゲーム木の展開
	while (!updateQueue.empty()) {

		const Board top = updateQueue.front();
		Node& now = m[top];
		updateQueue.pop();
	...
	for (const auto& n : m) {
		if (n.second.isLeaf) {
			for (int i = 0; i < n.second.parents.size(); i++)
				updateQueue.push(n.second.parents[i]);
		}
	}
	...
	///////////結果及びゲーム木の表示
	std::cout << "ゲーム全体は勝ち(0<a)or負け(a<0)or引き分け(a=0)です。:" << m[startHashValue].gameValue << std::endl;
	{
		int depthList[N * N + 1] = {};
		for (const auto& n : m) {

			if (n.second.hasGameValue)
				countHasGameValue++;
			else {
				countDoesntHaveGameValue++;
			}

			depthList[n.second.depth]++;

			if (false) {//結果の表示が必要なときは付けて。ただしかなり出ます。
				TicTacToeGame::DisplayCSV(n.second, std::cout);
			}
	...
	////////////自動対戦する
	{
		...
		for (int f = 0; f < 3; f++) {
			for (int s = 0; s < 3; s++) {
				...
				std::unique_ptr<PlayerBase> fp;
				std::unique_ptr<PlayerBase> sp;

				for (int i = 0; i < count; i++) {

					if (f == 0) {
						fp = std::unique_ptr<PlayerBase>(new WeekAI());
					}
					else if (f == 1) {
						fp = std::unique_ptr<PlayerBase>(new RandomAI());
					}
					else if (f == 2) {
						fp = std::unique_ptr<PlayerBase>(new StrongAI());
					}

					if (s == 0) {
						sp = std::unique_ptr<PlayerBase>(new WeekAI());
					}
					else if (s == 1) {
						sp = std::unique_ptr<PlayerBase>(new RandomAI());
					}
					else if (s == 2) {
						sp = std::unique_ptr<PlayerBase>(new StrongAI());
					}
					GameState gs = TicTacToeGame::AutoPlay(m, std::move(fp), std::move(sp));
					play_data[f][s][(int)gs]++;
				}
			}
		}


		std::cout << "{WHITE_WIN,DRAW,BLACK_WIN}" << std::endl;
		for (int f = 0; f < 3; f++) {
			for (int s = 0; s < 3; s++) {
